local utils <const> = {version = "2.4.1"}
...
function utils.draw_outline(v2pos, v2size, color, thickness)
    ...
    scriptdraw_draw_rect(
        utils.memoize.v2(v2pos.x, v2pos.y - (v2size.y/2)),
        utils.memoize.v2(v2size.x + thickness_x, thickness_y),
        color
    )
    ...
    scriptdraw_draw_rect(
        utils.memoize.v2(v2pos.x - (v2size.x/2), v2pos.y),
        utils.memoize.v2(thickness_x, v2size.y - thickness_y),
        color
    )
...
function utils.get_key(...)
	...
	return utils.Keys[ID], ID
end
--

function utils.new_reusable_v2(limit)
	limit = limit or 2
...
-- Do Key
do
	local get_key <const> = utils.get_key
	local key_waits = {}
	...
	function utils.make_key_wait_func(key_wait_table, first_wait, repeat_wait)
		local object = {
	...
	function utils.get_key_wait(vk, key_wait_table, first_wait, repeat_wait)
		first_wait = first_wait or 500
		repeat_wait = repeat_wait or 100
		key_wait_table = key_wait_table or key_waits
...
do
	local textv2 = v2(2, 2)
	local reuse_v2 = utils.new_reusable_v2()
	...
	local function draw_selector(object)
		while true do
			local selected = object.selected
			scriptdraw_draw_rect(reuse_v2(0, 0), textv2, 0x7D000000)
			scriptdraw.draw_text(object.selected_str, reuse_v2(-0.02, 0), textv2, 1 / object.text_size_rel_to_res, 0xFFFFFFFF, 1 << 4)

			if object.move then
				local is_going_up = object.move == "up"
				if (is_going_up and object.selected - 1 >= 1) or (not is_going_up and object.selected + 1 <= #object.items) then
					object.offset = is_going_up and object.offset - object.offset_step or object.offset + object.offset_step
					object.size_offset = object.size_offset + object.size_step
					if object.offset >= 0.08 or object.offset <= -0.08 then
						object.offset = 0
						object.size_offset = 0
						object.next_selected = 0

						object.selected = is_going_up and object.selected - 1 or object.selected + 1
			...
			local max = selected + 5 <= #object.items and selected + 5 or #object.items
			for i = min, max do
				if object.items[i] then
					local size = i == selected and 1.3 or 1
					size = i == object.next_selected and size + object.size_offset or i == selected and size - object.size_offset or size
					local alpha = object.move == "up" and math.max(math.floor(255 - math_abs((i - selected) + math_abs(object.offset)/0.08)*51), 0) or math.max(math.floor(255 - math_abs((i - selected) - math_abs(object.offset)/0.08)*51), 0)
					scriptdraw.draw_text(object.items[i], reuse_v2(0, 0.005 - 0.08 * (i - selected) + (object.offset or 0)), textv2, size / object.text_size_rel_to_res, (alpha << 24 | 0xFFFFFF), 0)
				end
			end
	...
	function utils.selector(selected_str, speed, index, items)
		...
		local drawThread = menu.create_thread(draw_selector, object)
		...
		while true do
			...
			if get_key_wait("up") then
				object.move = "up"
				object.next_selected = object.selected - 1
			end
		...
		return object.selected, object.items[object.selected]
	end
end
--
...
do
	...
	function utils.draw_slider(pos, width, min, max, value, colorBG, colorActive, colorText, draw_value)
...
do
	local v2r = utils.new_reusable_v2(4)
	function utils.draw_rect_ext_wh(pos, size, color1, color2, color3, color4)
...
do
	...
	local v2r		<const> = utils.new_reusable_v2()

	---@param pos v2
	---@param size v2
	---@param radius number float from 0 to 1
	---@param color uint32_t|number
	function utils.draw_rounded_rect(pos, size, radius, color)
...
Usage example:
	...
	local window_test = utils.new_side_window("test_header", v2(0.5, 0.5))
	...
	menu.add_feature("hide field", "toggle", 0, function(f)
		if f.on then
			window_test:remove_field(window_test:get_field_by_name("field"))
			firstField:set_value("right")
		else
			window_test:add_field("field", "menu", 2)
			firstField:set_value("field enabled")
		end
	end).on = true
]]
do
	local function draw_side_window(self)
		...
		scriptdraw_draw_rect(self.pos, utils.memoize.v2(self.rect_width, rect_height), self.rect_color)
		if self.header_on then
			scriptdraw_draw_rect(utils.memoize.v2(self.pos.x, header_y), utils.memoize.v2(self.rect_width, self.header_height), self.rect_color)
		end

		local text_size = graphics.get_screen_width()*graphics.get_screen_height()/3686400*0.75+0.25
		-- Header text
		scriptdraw.draw_text(self.header_text, utils.memoize.v2(self.pos.x - self.header_text_width/graphics.get_screen_width(), header_y+0.015), utils.memoize.v2(2, 2), text_size, self.text_color, 0, 0)
		-- table_of_lines
		for id, field in ipairs(self.fields) do
			if field.hidden then
				...
				scriptdraw.draw_text(field.name, utils.memoize.v2(self.pos.x-self.rect_width/2+self.text_padding, pos_y), utils.memoize.v2(2, 2), text_size, self.text_color, 0, 2)
				scriptdraw.draw_text(field.value, utils.memoize.v2(self.pos.x+self.rect_width/2-self.text_padding, pos_y), utils.memoize.v2(2, 2), text_size, self.text_color, 16, 2)
			end
		end
	end
	...
	---@return window
	function utils.new_side_window(header_text, pos, rect_color, rect_width, text_spacing, text_padding, text_color)
		local text_size = graphics.get_screen_width()*graphics.get_screen_height()/3686400*0.75+0.25
...
do
	...
	local function convert_range(self, value)
		return self.base_value + (value * self.step) -- this calculation
	end
	local Metatable = {__call = convert_range}

	-- Using this to simplify the calculation above ^ makes it more efficient than running this calculation ↓ every time
	local function update_range(self, original_range_min, original_range_max, convert_range_min, convert_range_max)
		...
		self.base_value = (0-original_range_min)/adjusted_max_original*adjusted_max_convert+convert_range_min -- this
		self.step = (1-original_range_min)/adjusted_max_original*adjusted_max_convert+convert_range_min - self.base_value -- and this
	end

	--[[
		local rng_cnvrt = utils.create_range_converter(0, 1, 0, 100)
		
		rng_cnvrt(5) -- returns 500
	]]
	---@return range_converter
	function utils.create_range_converter(original_range_min, original_range_max, convert_range_min, convert_range_max)
		local object = {
			update_range = update_range
		}

		object:update_range(original_range_min, original_range_max, convert_range_min, convert_range_max)
...
function utils.grid_pos(start_pos, offset, place, limit, flag)
	place = place - 1
	flag = flag or 0
...
do
	local grid_pos <const> = utils.grid_pos
	local start_pos
	local offset
	local limit
	local flag
	...
	function utils.iter_grid(istart_pos, ioffset, ilimit, iflag, istart_place, imod, imax)
...
-- Find Longest Match
function utils.find_longest_match(search_str, find_str, case_insensitive)
...
do
	...
	function utils.wrap_text(text, font, scale, relWidth)
...
-- Convert Colors
do
	function utils.convert_rgba_to_int(r, g, b, a)
	...
	function utils.convert_int_to_rgba(...)
...
-- Round Numbers
function utils.round_num(n, digits)
...
function utils.hsv_to_rgb(hue, sat, val) -- This was mostly taken from wikipedia
	...
	for n = 1, 5, 2 do
		local k = (n + k) % 6
		rgb[#rgb+1] = utils.round_num((val - val * sat * math.max(0, math.min(k, 4-k, 1))) * 255)
	end
...
function utils.rgb_to_hsv(r, g, b) -- This was mostly taken from wikipedia
...
function utils.controls_get_normal(...)
...
do
	...
	local mousev2					= v2()
	local mousev2r					= utils.new_reusable_v2(2)
	...
	function utils.mouse.enable(draw)
		...
		mousev2.x, mousev2.y = utils.controls_get_normal(0, 239)*2-1, utils.controls_get_normal(0, 240)*-2+1 - mouse_vertical_offset
		if draw then
	...
	local function update_slider(self, disable_control)
		...
		local mouse_x, mouse_y = utils.controls_get_normal(0, 239)*2-1, utils.controls_get_normal(0, 240)*-2+1
		if utils.control_is_just_pressed(0, 142) and not disable_control then
			self.within_hitbox = self:is_within_hitbox()
		elseif not utils.control_is_pressed(0, 142) then
	...
	local function mouse_slider(sType, pos, hitbox, size, default_x, default_y)
		...
		object.range['hitbox_x'] = utils.create_range_converter(pos.x+hitbox.x/2, pos.x-hitbox.x/2, 0, 1)
		object.range['hitbox_y'] = utils.create_range_converter(pos.y+hitbox.y/2, pos.y-hitbox.y/2, 0, 1)

		if sType & 1 ~= 0 then
			object.range['x'] = utils.create_range_converter(pos.x-size.x/2, pos.x+size.x/2, 0, 1)
			object.range['inverse_x'] = utils.create_range_converter(0, 1, pos.x-size.x/2, pos.x+size.x/2)
		end
		if sType >> 1 & 1 ~= 0 then
			object.range['y'] = utils.create_range_converter(pos.y-size.y/2, pos.y+size.y/2, 0, 1)
			object.range['inverse_y'] = utils.create_range_converter(0, 1, pos.y-size.y/2, pos.y+size.y/2)
		end
	...
	---@return mouse_slider
	function utils.mouse.horizontal_slider(pos, hitbox, size, default_x)
	...
	---@return mouse_slider
	function utils.mouse.vertical_slider(pos, hitbox, size, default_y)
	...
	---@param default_y number|nil 		y
	---@return mouse_slider
	function utils.mouse.xy_slider(pos, hitbox, size, default_x, default_y)
	...
	local function update_button(self, disable_control)
		local is_just_pressed = utils.control_is_just_pressed(0, 142)
	...
	---@return button
	function utils.mouse.button(text, pos, --[[size,]] bg_color, text_color, hover_bg_color, hover_text_color, font, text_scale, text_shadow, callback_function)
		text = tostring(text)
		...
		circle_x = scriptdraw_size_pixel_to_rel_x(circle_x)/2
		button_object.range['hitbox_x'] = utils.create_range_converter(pos.x+size.x/2+circle_x, pos.x-size.x/2-circle_x, 0, 1)
		button_object.range['inverse_x'] = utils.create_range_converter(0, 1, pos.x+size.x/2, pos.x-size.x/2)
		button_object.range['hitbox_y'] = utils.create_range_converter(pos.y+size.y/2, pos.y-size.y/2, 0, 1)
		button_object.range['inverse_y'] = utils.create_range_converter(0, 1, pos.y+size.y/2, pos.y-size.y/2)
	...
	-- Input Box
	local keys <const> = {
		enter		= utils.get_key(0x0D),
		shift		= utils.get_key(0x10),
		ctrl		= utils.get_key(0x11),
		paste		= utils.get_key(0x56, 0x11),
		copy		= utils.get_key(0x43, 0x11),
		esc			= utils.get_key(0x1B)
	}
	...
	local function draw_input_box(self)
		...
		utils.draw_outline(self.pos, rect_size, self.color.outline or 0xDDAAAAAA, 2)
	...
	local copy_paste_do_key = utils.make_key_wait_func(key_waits, 10000)
	local function update_input_box(self, disable_control)
		...
		if utils.control_is_just_pressed(0, 142) and not disable_control and self:is_within_hitbox() then
		...
		if self.active then
			...
			if utils.get_key_wait(0x25, key_waits) then -- left
				self.cursor = self.cursor - 1
			elseif utils.get_key_wait(0x27, key_waits) then -- right
				self.cursor = self.cursor + 1
			end
			...
			if utils.get_key_wait(0x08, key_waits, nil, 75) and self.cursor ~= 1 then
			...
			for key_code, str_table in pairs(self.keys) do
				if #self.str < self.limit and utils.get_key_wait(key_code, key_waits) and not keys.ctrl:is_down() then
	...
	function utils.input_box(pos, width, type, scale, font, limit, padding, placeholder)
		local object = {
			char				= {""},
			str					= "",
			pos					= pos,
		...
		object.padding:size_pixel_to_rel()
		object.text_size:size_pixel_to_rel()

		object.range['hitbox_x']		= utils.create_range_converter(pos.x+object.width/2+object.padding.x, pos.x-object.width/2-object.padding.x, 0, 1)
		object.range['inverse_x']	= utils.create_range_converter(0, 1, pos.x+object.width/2+object.padding.x, pos.x-object.width/2-object.padding.x)
		object.range['hitbox_y']		= utils.create_range_converter(pos.y+object.text_size.y/2+object.padding.y, pos.y-object.text_size.y/2-object.padding.y, 0, 1)
		object.range['inverse_y']	= utils.create_range_converter(0, 1, pos.y+object.text_size.y/2+object.padding.y, pos.y-object.text_size.y/2-object.padding.y)

		local pasteCheck <const> = {}
		for _, v in pairs(object.keys) do
		...
		object.paste_check = "[^"..table.concat(pasteCheck):gsub(".", "%%%1").."]"
...
--[[
	Example:
		...
		repeat
			status, ABGR, red, green, blue, alpha = utils.pick_color()
			if status == 2 then
				return
			end
			system.wait(0)
...
do
	utils.size_scale = (graphics.get_screen_width()*graphics.get_screen_height()-921600) / (8924400-921600) * (1.25) + 0.85
	local size_scale = utils.size_scale
	local alpha_slider
	local hue_slider
	local color_picker = utils.mouse.xy_slider(
		v2(),
	...
	color_picker:set_draw_function(function(slider)
		...
		local b, g, r = utils.hsv_to_rgb(hue)
		local color = utils.convert_rgba_to_int(r, g, b)

		utils.draw_rect_ext_wh(slider.pos, slider.size, 0xFFFFFFFF, 0xFFFFFFFF, color, color)
		utils.draw_rect_ext_wh(slider.pos, slider.size, 0xFF000000, 0, 0, 0xFF000000)
		...
		b, g, r = utils.hsv_to_rgb(hue, sat, val)

		slider.color = utils.convert_rgba_to_int(r, g, b, a)
		slider.colors[1] = r
		slider.colors[2] = g
		slider.colors[3] = b
		slider.colors[4] = a
	...
	hue_slider = utils.mouse.vertical_slider(v2(color_picker.pos.x+color_picker.size.x/2+scriptdraw_size_pixel_to_rel_x(50*size_scale), color_picker.pos.y-scriptdraw_size_pixel_to_rel_y(52*size_scale)), size, size, 0)
	hue_slider.hue = 0

	local hue_gradient = {}
	local hue_y = hue_slider.pos.y-hue_slider.size.y/2+scriptdraw_size_pixel_to_rel_y(30*size_scale)
	for i = 0, 6 do
		local b, g, r = utils.hsv_to_rgb(i*60)
		local bottom = utils.convert_rgba_to_int(r, g, b)

		b, g, r = utils.hsv_to_rgb((i+1)*60)
		local top = utils.convert_rgba_to_int(r, g, b)
	...
	hue_slider:set_draw_function(function(slider)
		for i = 1, 6 do
			...
			utils.draw_rect_ext_wh(slider.v2r(slider.pos.x, hue_table.y), hue_size, hue_table.bottom, hue_table.top, hue_table.top, hue_table.bottom)
		end
		...
		local b, g, r = utils.hsv_to_rgb((slider.value.y)*360)
		local color = utils.convert_rgba_to_int(r, g, b)
	...
	alpha_slider = utils.mouse.horizontal_slider(
	...
	local function set_rgba_values(r, g, b, a)
		if (r and g and b) then
			local hue, sat, val = utils.rgb_to_hsv(r, g, b)
	...
	-- Buttons
		...
		do
			...
			for i = 1, 12 do
				local button <const> = utils.mouse.button(
					"",
					v2(utils.grid_pos(pos, offset, i, 6, 0)),
		...
		local hex_button_thread_func = function(button)
			...
			if button.data ~= 0 then
				local r, g, b, a = utils.convert_int_to_rgba(color_picker.color, 'r', 'g', 'b', 'a')
				local hex = string.format("%08X", utils.convert_rgba_to_int(b, g, r, a))
		...
		local hex_button = utils.mouse.button("#DDDDDD", text_pos, 0, 0xFFFFFFFF, 0x33000000, nil, nil, size_scale, true, function(button)
			...
			if button.data ~= 0 and utils.time_ms() - button.data < 250 then
				...
				local r, g, b, a = utils.convert_int_to_rgba(color_picker.color, 'r', 'g', 'b', 'a')
				local default_str = string.format("%08X", utils.convert_rgba_to_int(b, g, r, a))
				local code, hex
				...
				local new_alpha
				r, g, b, new_alpha = utils.convert_int_to_rgba(hex, 'b', 'g', 'r', 'a')
		...
		local apply_button = utils.mouse.button(
			"Apply",
		...
		local cancel_button = utils.mouse.button(
			"Cancel",
			v2(color_picker.pos.x-color_picker.size.x/4-(scriptdraw_size_pixel_to_rel_x(14)*size_scale), color_picker.pos.y-color_picker.size.y),
			0xDD000000, 0xDDFFFFFF, 0xDDFFFFFF, 0xDD000000, nil, size_scale, nil
		)
	...
	---@param pos v2
	function utils.set_color_picker_pos(pos)
		...
		do
			...
			for i = 2, 12 do
				local x <const>, y <const> = utils.grid_pos(prev_pick_pos, offset, i, 6, 0)
				v2r.x, v2r.y = x, y
	...
	function utils.pick_color(r, g, b, a)
		if not running then
			set_rgba_values(r, g, b, a)
			while utils.get_key(0x0D):is_down() or utils.get_key(0x1B):is_down() or utils.get_key(0x08):is_down() do
				system.wait(0)
			end
		end

		if uIdata then
			uIdata.inputBoxOpen = true
		end
		running = true
		--menu.set_menu_can_navigate(false)
		...
		if lastIntColor ~= color_picker.color then
			local r, g, b = utils.convert_int_to_rgba(color_picker.color, "r", "g", "b")
		...
		utils.mouse.enable(true)

		if (utils.get_key(0x0D):is_down() or utils.get_key(0x1B):is_down() or utils.get_key(0x08):is_down()) or (apply or cancel) then
			local success = utils.get_key(0x0D):is_down() or apply
			while utils.get_key(0x1B):is_down() or utils.control_is_pressed(0, 142) do
				utils.mouse.enable()
				controls.disable_control_action(0, 200, true)
				system.wait(0)
			end
